...
\chapter{Some ideas in partial differential equations}
...
$\Psi$ having \textit{compact support}, where $\Phi$ and $\Psi$ typically consist of continuous functions in
some Sobolev space. We will formally define a ``finite element method'', following Ciarlet \cite{ciarlet},
...
\begin{align*}
    \mu\Delta u + \rho g - \nabla p = 0,\quad \nabla\cdot u = 0,
\end{align*}
form a constrained linear equation.
...
As we start with the \textit{differential} equation \eqref{steady_stokes}, we can introduce a trial space $\Psi$ and then ``weaken''
...
\begin{equation}\label{steady_stokes_weak}
    \int_\Omega \left(\mu\Delta u + \rho g\right)v\,dx = 0
    \quad\equiv\quad
    \int_\Omega -\mu\nabla u\cdot \nabla v\,dx = \int_\Omega \rho gv\,dx.
\end{equation}

\subsection{Forming a linear system}
Noting that the left-hand-side of \eqref{steady_stokes_weak} is a bilinear form in $u$ and $v$, and the right-hand-side
...
\begin{equation}\label{elliptic_bilinear_form_matrix}
    ...
    = \hat{f}.
\end{equation}
It should be noted that each term in $\eqref{elliptic_bilinear_form}$ is a vector of the dimension of the domain $\Omega$
(as $u$ is a velocity field),
and therefore $A$ and $\hat{f}$ in \eqref{elliptic_bilinear_form_matrix} have vector entries.
