...
\begin{equation}\label{steady_stokes_dirichlet_energy}
    E(u) \coloneqq \int_{\Omega} \frac{\mu}{2} \norm{\nabla u}^2 - \rho g\cdot u \,dx = \int_{\Omega} \fancyL\,dx.
\end{equation}
...
\begin{equation}\label{steady_stokes_unconstrained_weak}
    \int_\Omega -\mu\Delta uv\,dx = \int_\Omega \rho gv\,dx
    \quad\equiv\quad
...
\end{align*}
This is least-squares. Letting $P = I - \frac{cc^T}{c^Tc}$, which orthogonally projects the $c$ component out of vectors,
we have an equivalent system
\begin{align*}
    PA\hat{u} - P\hat{f} = 0,
\end{align*}
which is full-rank when restricted to the subspace orthogonal to $c$ (the space that satisfies the constraint $c^Tx = 0$).
