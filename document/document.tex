...
A diffusion process ``levels out'' some quantity, such as heat or chemical concentration. We can intuitively think of a diffusion as a progressive ``blurring''
such as is seen in a camera defocus, and in fact many common image processing techniques use diffusion PDEs from physics. We will stick with
...
\begin{equation}\label{heat_equation_differential_constant}
    \frac{dh}{dt} = s + \mu\nabla \cdot \nabla h = s + \mu\Delta h,
\end{equation}
which is the standard heat equation, and simplest example of the use of the Laplacian.
The steady-state heat equation is then
\begin{equation}\label{heat_equation_steady_state}
    -\Delta h = s/\mu.
\end{equation}



It is a standard model of diffusion processes which leads directly to the Laplacian.



$$\int_{\pomn} -\nabla u \cdot \hat{n}\,dx = \int_{\omn}\,dx$$
