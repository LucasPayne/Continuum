...
% \end{align*}
We can solve \eqref{elliptic_bilinear_form_matrix} to get a velocity field $u$, although in general this will not satisfy $\nabla\cdot u = 0$.
...
\begin{split}
    &\int_\om \mu\nabla u\cdot\nabla v + \left(\nabla\cdot p\right)v\,dx = \int_\om -\rho g\cdot v\,dx,\\
    &\int_\om \left(\nabla\cdot u\right) q\,dx = 0, \quad\text{where $v \in \Psi, q \in \trialconstraint$}.
\end{split}
\end{equation}
