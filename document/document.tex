...
\section{Stokes flow and the meaning of pressure}
...
\begin{equation}\label{steady_stokes_dirichlet_energy}
    E(u) \coloneqq \int_{\Omega} \frac{\mu}{2} \norm{\nabla u}^2 - \inner{\rho g, u} \,dx = \int_{\Omega} \fancyL\,dx.
\end{equation}
...
\begin{equation}\label{virtual_pressure_lambda}
...
the virtual displacements maintain the constraint of incompressibility), we get the form
\begin{align*}
...
we can substitute $-p$ into \eqref{virtual_pressure_lambda} to get
\begin{equation}\label{virtual_pressure}
    \inner{\rho g + \mu\Delta u, \delta u} = \nabla\cdot \left(p \delta u\right),\quad \nabla\cdot u = 0
\end{equation}
for unconstrained test functions $\delta u$.
---discuss hydrostatics, gravity.
...
A diffusion process ``levels out'' some quantity, such as heat or some chemical concentration. We can intuitively think of a diffusion as a progressive ``blurring''
...
the notion of heat $h$ as the diffused quantity.
\textit{Fick's law of diffusion} is a constitutive relation giving the bulk flux of heat $h$ as proportional to the negative gradient:
    $$hj = -\mu\nabla h,$$
where $\mu$ is called the diffusion coefficient.
This is one way of saying that heat tends to level out.
If we form a continuity equation \eqref{continuity_equation} for heat, with source $s$, we get
...
Formally, our basis functions must be in the Sobolev space $H^1$. This simply means that they must have a gradient defined ``almost everywhere''.
It does not matter if the gradient is not defined at isolated lower-dimensional subsets, as these make no contribution to
the integral. Since $f$, our domain decomposition $\Omega_j$, and the basis functions $\phi_i$ are known, we can pre-compute
...
\begin{equation}\label{poisson_matrix_equation}
    A\hat{h} = \begin{bmatrix}
            \integralentry{1}{1} & \cdots & \integralentry{1}{2} \\
            \vdots & & \vdots \\
...
\subsection{The notion of a trial function}
...
\begin{align*}
    ...
    \int_{\om_1}f\,dx,
\end{align*}
and so on. We cannot quantify flux integrals
over arbitrary control volumes, but we can take a number of ``trial'' flux integrals over the finite number of cells $\Omega_i$.
...
\begin{equation}
    ...
    \int_{\om_1 + \om_2}f\,dx.
\end{equation}
Formally $\Omega_1 + \Omega_2$ is called a \textit{chain}. For example, we may visualize $\Omega_1 + 2\Omega_2 + 0.5\Omega_4$ as:
...
\begin{equation}
    \inner{-\nabla \left(\sum_{i=1}^n h_i\phi_i\right), \partial(\om_1 + \om_2)}
    =
    \inner{f, \om_1 + \om_2}.
\end{equation}

The Poisson equation is discretized over flux integrals around cell boundaries in the linear system \eqref{poisson_equation_integral_discretized},
...
\end{equation}
and we see that we still only require the $\phi_i$ to be in $H^1$.
...
While this is more readily formalized with the notion of a distribution, or generalised function, we will work with the Dirac delta function.
For example, possibly under some regularity assumptions, we may represent a function of a real variable as
    $$f = \int_{-\infty}^\infty \delta_x f(x)\,dx,$$
which we may call ``Riemann-integral-like''.
We may instead represent $f$ in a ``Lebesgue-integral-like'' way

For example, if we let $\Psi$ be spanned by the hat functions on some triangulation, each hat function $\psi_i$
will be 
